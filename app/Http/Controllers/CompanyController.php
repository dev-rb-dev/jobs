...
use App\Queries\EmployeeFollowerDataTable;
use App\Queries\ReportedCompanyDataTable;
use App\Queries\ReportedEmployeeDataTable;
...
class CompanyController extends AppBaseController
{
    ...
    {
        if ($request->ajax()) {
            return Datatables::of((new ReportedEmployeeDataTable())->get())->make(true);
        }
    ...
    {
        if ($request->ajax()) {
            return DataTables::of((new EmployeeFollowerDataTable())->get())->make(true);
        }
    ...
    public function markAsFeatured($companyId)
    {
        ...
        if ($isFeaturedAvailable) {
            ...
            if ($notificationStatus['MARK_JOB_FEATURED'] == Company::ISACTIVE) {
                NotificationSetting::where('key', 'MARK_COMPANY_FEATURED_ADMIN')->where('type',
                    'admin')->first()->value == 1 ?
                    addNotification([
