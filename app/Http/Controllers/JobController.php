...
class JobController extends AppBaseController
{
    ...
    public function index(Request $request ,$jobStatus=null)
    {
    //    dd($request);
        if($request->ajax()) {
        ...
        return view('employer.jobs.index',
            compact('jobStatus','statusArray', 'isFeaturedEnable', 'isFeaturedAvilabal', 'featured', 'admin_approved'));
    }
    ...
    public function store(CreateJobRequest $request)
    {
        ...
        $input['status'] = ($request->saveDraft) ? Job::STATUS_DRAFT : Job::STATUS_OPEN;
         // dd($input);
    ...
    public function getJobs(Request $request)
    {
        ...
        $admin_approved = Job::ADMIN_APPROVED;
        return view('jobs.index', compact('featured', 'suspended', 'freelancer', 'jobsActiveExpire','admin_approved'));
    }
    ...
    {
        if ($request->ajax()) {
            return Datatables::of((new ReportedJobsDataTable())->getReportedJob($request->only([
 ...
 public function makeFeatured($jobId)
    {
        ...
        if ($isFeaturedAvailable) {
            ...
            if ($notificationStatus['MARK_COMPANY_FEATURED'] == Job::STATUS_OPEN) {
                NotificationSetting::where('key','MARK_JOB_FEATURED_ADMIN')->where('type',
                    'admin')->first()->value == 1 ?
                    addNotification([
    ...
    public function getPublishJobs(Request $request)
    {
        if ($request->ajax()) {
            return Datatables::of((new JobPublishedDataTable())->getJobs())->make(true);
        }
        $admin_approved = Job::ADMIN_APPROVED;
        return view('job_published.index', compact ('admin_approved') );
    }
    ...
    public function approveJob(Request $request)
    {
        ...
        $approve = isset($request->approve)?1:2;
        $job->admin_approved = $approve; //Approved
        $job->remarks= $request->remarks?$request->remarks:'';
        $job->save ();
        return redirect()->route('admin.jobs.index');
        //return view('job_published.index');
    }

    public function showremarks(Request $request, $jobid){
