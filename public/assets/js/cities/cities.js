...
$(document).ready(function () {
  ...
  var tableName = '#cityTbl';
  var tbl = $('#cityTbl').DataTable({
    processing: true,
    serverSide: true,
    searchDelay: 500,
    'order': [[0, 'asc']],
    ajax: {
      ...
      data: function data(_data) {
        _data.stateId = $('#filter_state').find('option:selected').val();
      }
    },
    columnDefs: [{
      'targets': [2],
    ...
    $.ajax({
      ...
      success: function success(result) {
        if (result.success) {
          ...
          $('#addCityModal').modal('hide'); // window.livewire.emit('refresh');
