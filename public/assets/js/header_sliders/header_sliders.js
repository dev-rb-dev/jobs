...
$(document).on('submit', '#addNewForm', function (e) {
  e.preventDefault();
  processingBtn('#addNewForm', '#btnSave', 'loading'); // if ($('#description').summernote('isEmpty')) {
  //     $('#description').val('');
  // }
    ...
    success: function success(result) {
      if (result.success) {
        ...
        if (isEmpty(result.data.header_slider_url)) {
          ...
          $('#editPreviewImage').css('background-image', 'url("' + result.data.header_slider_url + '")'); // attr('src', result.data.header_slider_url);

          $('#imageSliderUrl').css('background-image', 'url("' + result.data.header_slider_url + '")'); // attr('href', result.data.header_slider_url);
...
var tableName = '#headerSlidersTbl';
var tbl = $('#headerSlidersTbl').DataTable({
  processing: true,
  serverSide: true,
  'order': [3, 'desc'],
  ajax: {
    ...
    data: function data(_data) {
      _data.is_active = $('#headerFilterStatus').find('option:selected').val();
    }
  },
  columnDefs: [{
    'targets': [0],
  ...
  columns: [{
    data: function data(row) {
      return "<div class=\"d-flex align-items-center\">\n                            <div class=\"symbol symbol-circle symbol-50px overflow-hidden me-3\">\n                                <a >\n                                    <div class=\"\">\n                                        <img src=\"".concat(row.header_slider_url, "\" alt=\"\" class=\"user-img\">\n                                    </div>\n                                </a>\n                           </div>\n                         </div>");
    },
    ...
    data: function data(row) {
      ...
      return prepareTemplateRender('#headerSliderActionTemplate', data);
    },
    name: 'id'
  }, {
    data: 'created_at',
