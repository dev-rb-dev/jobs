...
$(document).on('submit', '#addNewForm', function (e) {
  ...
  var dataDesc = JSON.stringify(editor_content_1);
  $('#descriptionData').val(dataDesc.replace(/"/g, '')); // if (!checkSummerNoteEmpty('#description',
    ...
    success: function success(result) {
      if (result.success) {
        ...
        if (isEmpty(result.data.image_slider_url)) {
          ...
          $('#editPreviewImage').css('background-image', 'url("' + result.data.image_slider_url + '")'); // attr('src', result.data.image_slider_url);

          $('#imageSliderUrl').css('background-image', 'url("' + result.data.image_slider_url + '")'); // attr('href', result.data.image_slider_url);

          $('#imageSliderUrl').text(view);
        }

        element.innerHTML = result.data.description;
        quill2.root.innerHTML = element.value; // $('#editDescription').summernote('code', result.data.description);
...
$(document).on('submit', '#editForm', function (event) {
  ...
  var editor_content1 = quill2.root.innerHTML;
...
$('#addModal').on('hidden.bs.modal', function () {
  resetModalForm('#addNewForm', '#validationErrorsBox'); // $('#description').summernote('code', '');
  // $('#previewImage').attr('src', defaultDocumentImageUrl);

  $('#previewImage').css('background-image', 'url("' + defaultDocumentImageUrl + '")');
});
$('#editModal').on('hidden.bs.modal', function () {
  resetModalForm('#editForm', '#editValidationErrorsBox'); // $('#editDescription').summernote('code', '');
  ...
  $.ajax({
    ...
    success: function success(result) {
      if (result.success) {
        ...
        var status = result.data.is_active ? 'messages.common.active' : 'messages.common.de_active';
...
var quill1 = new Quill('#descriptionId', {
  modules: {
...
var quill2 = new Quill('#editDescriptionId', {
  modules: {
    toolbar: [['bold', 'italic', 'underline', 'strike'], ['clean']]
  },
  placeholder: 'Description',
...
var tbl = $('#imageSlidersTbl').DataTable({
  processing: true,
  serverSide: true,
  'order': [4, 'desc'],
  ajax: {
  ...
  columns: [{
    data: function data(row) {
      return "<div class=\"d-flex align-items-center\">\n                            <div class=\"symbol symbol-circle symbol-50px overflow-hidden me-3\">\n                                <a>\n                                    <div class=\"\">\n                                        <img src=\"".concat(row.image_slider_url, "\" alt=\"\" class=\"user-img\">\n                                    </div>\n                                </a>\n                           </div>\n                         </div>");
    },
