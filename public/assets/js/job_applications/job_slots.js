...
$(document).on('click', '.edit-btn', function (event) {
  ...
  $.ajax({
    ...
    success: function success(result) {
      if (result.success) {
        $('#editSlotId').val(result.data.id); // $('#editDate').val(moment(result.data.date).
        // format('YYYY-MM-DD'));

        document.querySelector('#editDate')._flatpickr.setDate(moment(result.data.date).format());

        document.querySelector('#editTime')._flatpickr.setDate(result.data.time);
...
$(document).on('click', '.cancel-slot', function (e) {
  e.preventDefault();
  var cancelSlotNote = $('.cancel-slot-notes').map(function () {
    if ($(this).val() != '') {
      return $(this).val();
    }
  }).get(); // if (cancelSlotNote == '') {
  //     displayErrorMessage('Cancel Reason field is required');
  //     return false;
  // }

  var slotID = $(this).data('schedule');
  $.ajax({
    url: cancelSlotUrl,
    type: 'POST',
    data: {
    ...
    success: function success(result) {
      if (result.success) {
        ...
        if (!$('.schedule-interview')[0]) {
          $('.slot-screen > div:eq(0)').after('<a href="javascript:void(0)" class="ml-4 btn btn-primary form-btn float-right schedule-interview">' + Lang.get('messages.common.add') + '</a>');
        } else {
...
var timePickerFunction = function timePickerFunction(selector) {
  $(document.getElementById(selector)).flatpickr({
    ...
    dateFormat: "H:i",
    time_24hr: true,
    minuteIncrement: 1 // defaultHour:0
