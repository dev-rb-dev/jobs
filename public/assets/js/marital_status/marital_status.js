...
$(document).ready(function () {
  ...
  var tbl = $('#marital_status').DataTable({
    processing: true,
    serverSide: true,
    searchDelay: 500,
    'order': [[0, 'asc']],
    ...
    columns: [{
      data: function data(row) {
        return '<a class="show-btn cursor-pointer" data-id="' + row.id + '">' + row.marital_status + '</a>';
      },
      name: 'marital_status'
    }, {
      data: function data(row) {
      ...
      data: function data(row) {
        var url = maritalStatusUrl + '/' + row.id;
        var data = [{
          'id': row.id,
          'url': url + '/edit'
        }];
  ...
  var quill = new Quill('#martialDescription', {
    modules: {
