...
$(document).on('change', '.isActive', function (event) {
  ...
  $.ajax({
    ...
    success: function success(result) {
      if (result.success) {
        displaySuccessMessage(result.message); //window.livewire.emit('refresh');
  ...
  var tbl = $('#noticeboardsTbl').DataTable({
    ...
    columns: [{
      data: function data(row) {
        return "<a class=\"show-btn\" href=\"javascript:void(0)\" data-id=\"".concat(row.id, "\">").concat(row.title.length > 20 ? row.title.substr(0, 20) + '...' : row.title, "</a>");
      },
      name: 'title'
    }, {
      data: function data(row) {
