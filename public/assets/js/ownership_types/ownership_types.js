...
$(document).ready(function () {
  ...
  var tbl = $('#ownershipTypeTbl').DataTable({
    processing: true,
    serverSide: true,
    searchDelay: 500,
    // 'order': [[1, 'asc']],
    ...
    columns: [{
      ...
      data: function data(row) {
        var url = ownerShipTypeUrl + '/' + row.id;
        var data = [{
          'id': row.id,
          'url': url + '/edit'
        }];
  ...
  $(document).on('submit', '#addOwnershipForm', function (e) {
    e.preventDefault(); // if (!checkSummerNoteEmpty('#ownershipDescription',
...
var quill = new Quill('#ownershipDescription', {
  modules: {
