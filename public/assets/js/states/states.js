...
$(document).ready(function () {
  ...
  var tableName = '#stateTbl';
  var tbl = $('#stateTbl').DataTable({
    processing: true,
    serverSide: true,
    searchDelay: 500,
    'order': [[0, 'asc']],
    ajax: {
      ...
      data: function data(_data) {
        _data.countryId = $('#filter_country').find('option:selected').val();
      }
    },
    columnDefs: [{
      'targets': [2],
  ...
  handleSearchDatatable(tbl); // $('#countryID').select2({
  //     'width': '100%',
  //     dropdownParent: $('#addStateModal'),
