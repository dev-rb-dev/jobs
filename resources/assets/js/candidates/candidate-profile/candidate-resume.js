...
$(document).ready(function () {
    let tableName = '#resumeTbl';
    let tbl = $('#resumeTbl').DataTable({
        processing: true,
        serverSide: true,
        searchDelay: 500,
        'order': [[1, 'asc']],
        ...
        columns: [
            {
                data: function (row) {
                    if (row.custom_properties.is_default === true) {
                        return `<span class="text-primary">${row.custom_properties.title + '(Default)'}</span>`;
                    }
            ...
            {
                data: function (row) {
                    let url = downloadResumeUrl + '/' + row.id;
                    let data = [
                        {
                            'id': row.id,
                            'downloadResume': url,
    ...
    $(document).on('click', '.delete-resume', function (event) {
        ...
        deleteItem(resumeUploadUrl + '/' + resumeId, tableName,
