...
let tbl = $(tableName).DataTable({
    ...
    columns: [
        ...
        },
        {
            data: function (row) {
                return `<div class="badge badge-light">
                    <div> ${moment(row.created_at).startOf('day').fromNow()}</div>
                    </div>`;
            },
            name: 'created_at',
        },
        {
            data: function (row) {
                return `<i class="${row.immediate_available == 1
        ...
        {
            data: function (row) {
                return `<i class="${row.user.email_verified_at == null
                    ? 'fas fa-times-circle text-danger'
                    : 'fas fa-check-circle text-success'} "></i>`;
            },
...
$(document).ready(function () {
    let companyTableName = '#companiesTbl';
    let companyTable = $(companyTableName).DataTable({
        processing: true,
        serverSide: true,
        dataType: 'json',
        'order': [[5, 'desc']],
        ...
        columns: [
            {
                data: function (row) {
                    ...
                    <div class="symbol symbol-50px me-2 d-flex align-items-center ms-5">
                       <div class="d-flex flex-column">
                <a href="${companiesUrl}/${row.id}" class="mb-1">${row.user.first_name}</a>
            </div>
                    </div>`;
                },
                name: 'user.first_name',
            ...
            },
            {
                data: function (row) {
                    if (row.website != null) {
                        return `<a href="${row.website}" class="mb-1"> ${row.website.length >
                        20
                            ? row.website.substr(0, 20) + '...'
                            : row.website}</a>`;
                    }
                },
                name: 'website',
            ...
            {
                data: function (row) {
                    return `<i class="${row.active_featured != null
                        ? 'fas fa-check-circle text-success'
                        : 'fas fa-times-circle text-danger'} "></i>`;
                },
                name: 'id',
