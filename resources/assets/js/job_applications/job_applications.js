...
let tbl = $('#jobApplicationsTbl').DataTable({
    processing: true,
    serverSide: true,
    'order': [[0, 'asc']],
    ajax: {
    ...
    columns: [
        ...
        },
        {
            data: function (row) {
                return row.job.currency.currency_icon + '&nbsp;' +
                    row.expected_salary.toLocaleString();
            },
        ...
        },
        {
            data: function (row) {
                ...
                return '<span class="badge badge-light-' + statusColor[row.status] +
    ...
    $(document).on('click', '.change-job-stage', function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $('#jobStageId').empty();
                $('#jobStageId').
                    append('<option value="">Select Job Stage</option>');
