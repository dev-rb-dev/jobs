        ...
        },
        {
            ...
            render: function (row) {
                if (!isEmpty(row.salary_currency)) {
                    ...
                    <div> ${row.salary_currency.currency_icon + ' ' +
                    currency(row.amount, {precision: 2}).format()}</div>
                    </div>`;
                } else {
                    return '$ ' +
                        `<div class="badge badge-light-success">
                    <div> ${currency(row.amount, {precision: 2}).format()}</div>
                    </div>`;
                }
            },
            name: 'amount',
        },
        {
            data: function (row) {
                return `<div class="badge badge-light-primary">
                    <div> ${row.active_subscriptions_count}</div>
                    </div>`;
            },
