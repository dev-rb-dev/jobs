<div class="d-md-flex d-block justify-content-between">
    <h1 id="candidateName" class="fw-bolder text-primary">{{ $user->full_name }}</h1>
...
<div class="mt-3">
    @isset($user->candidate->full_location)
        <p class="mb-1 fw-bolder text-muted" id="candidateLocation">{{ $user->candidate->full_location}}</p>
    @endisset
    <p class="mb-1 fw-bolder text-muted" id="cadidateEmail">{{ $user->email}}</p>
    <p id="candidatePhone" class="fw-bolder text-muted">{{ $user->phone}}</p>
</div>
    ...
    @if($user->candidateSkill)
        <ul class="pl-3">
            @foreach($user->candidateSkill as $skill)
                <li class="fw-bold text-muted">{{ $skill->name }}</li>
            @endforeach
        </ul>
    @endif
</div>
