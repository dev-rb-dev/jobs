...
{{--                @include('layouts.search-component')--}}
                <div class="card-toolbar">
                    <div class="d-flex align-items-center py-1">
                        <a class="btn btn-primary uploadResumeModal"><i
                                    class="fas fa-plus"></i>{{ __('messages.candidate_profile.upload_resume') }}</a>
                    </div>
                </div>
            </div>
            <div class="card-body pt-0 fs-6 py-8 px-8  px-lg-10 text-gray-700">
                <div class="table-responsive">
                    @include('candidate.profile.resume_table')
                </div>
            </div>
        </div>
@include('candidate.profile.modals.upload_resume_modal')
...
@push('scripts')
    <script>
        ...
        let candidateResumeUrl = "{{ route('candidate.profile',['section' => 'resumes']) }}";
        let downloadResumeUrl = "{{ route('download.resume') }}";
    </script>
