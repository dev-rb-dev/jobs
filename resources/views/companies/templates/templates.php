<script id="companyActionTemplate" type="text/x-jsrender">
 <a href="{{:url}}" title="<?php echo __('messages.common.edit') ?>" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 edit-btn">
...
<a class="btn btn-light btn-active-light-primary btn-sm dropdown-toggle"  data-bs-toggle="dropdown" aria-expanded="false">
  ...
  <ul class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 fw-bold fs-6 w-150px py-4 dropdown-menu" aria-labelledby="dropdownMenuButton1">
    <li><a class="btn btn-sm adminMakeFeatured" data-id="{{:id}}"><?php echo __('messages.front_settings.make_featured') ?></a>
    </li>
  </ul>
  </div>
   {{else}}
   <div title="Expries On {{:expiryDate}}" data-bs-toggle="tooltip">
  ...
  <ul class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 fw-bold fs-6 w-150px py-4 dropdown-menu" aria-labelledby="dropdownMenuButton1">
    <li> <a class="btn btn-sm adminUnFeatured" data-id="{{:id}}"><?php echo __('messages.front_settings.remove_featured') ?></a>
    </li>
  </ul>
   {{/if}}
...
<script id="isActive" type="text/x-jsrender">
   <label class="form-check form-switch form-check-custom form-check-solid form-switch-sm justify-content-center">
        <input type="checkbox" name="Is isActive" class="form-check-input isActive" data-id="{{:id}}" {{:checked}}>
        <span class="custom-switch-indicator"></span>
    </label>
</script>
<script id="isEmailVerified" type="text/x-jsrender">
 {{if !email_verified}}
   <label class="form-check form-switch form-check-custom form-check-solid form-switch-sm justify-content-center">
        <input type="checkbox" name="Is isActive" class="form-check-input is-email-verified" data-id="{{:id}}">
        <span class="custom-switch-indicator"></span>
    </label>
     {{else}}
     <div>
     <a title="<?php echo __('messages.common.resend_verification_mail')?>"
                       class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 edit-btn send-email-verification" data-id="{{:id}}">
                        <i title="<?php echo __('messages.common.resend_verification_mail')?>" class="fa fa-sync"></i>
                    </a>
                    </div>
                        {{/if}}
</script>
